...
type Props = {
  policyId: string;
  initialData?: ReservationCancellationPolicyDateRange;
  onSubmit: (
    data:
      | CreateReservationCancellationPolicyDateRangeDto
      | UpdateReservationCancellationPolicyDateRangeDto
...
const ReservationCancellationPolicyDateRangeForm: React.FC<Props> = ({
  policyId,
  initialData,
  onSubmit,
  isLoading = false,
  ...
  const validate = (): boolean => {
    ...
    if (!dateAfter.trim()) {
      newErrors.dateAfter = t(
        'reservationCancellationPolicyDateRanges.form.errors.dateAfterRequired'
      );
    }

    if (!dateBefore.trim()) {
      newErrors.dateBefore = t(
        'reservationCancellationPolicyDateRanges.form.errors.dateBeforeRequired'
      );
    }

    if (dateAfter && dateBefore && new Date(dateAfter) > new Date(dateBefore)) {
      newErrors.dateBefore = t(
        'reservationCancellationPolicyDateRanges.form.errors.dateBeforeAfter'
      );
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        <Label htmlFor="dateAfter" required>
          {t('reservationCancellationPolicyDateRanges.form.dateAfter')}
        </Label>
        <Input
          id="dateAfter"
          type="date"
      ...
      <div>
        <Label htmlFor="dateBefore" required>
          {t('reservationCancellationPolicyDateRanges.form.dateBefore')}
        </Label>
        <Input
          id="dateBefore"
          type="date"
...
export default ReservationCancellationPolicyDateRangeForm;
